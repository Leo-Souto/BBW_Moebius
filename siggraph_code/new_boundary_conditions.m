function [b,bc] = new_boundary_conditions(V,F,C,P,E,CE,CB,CCE)
  % BOUNDARY_CONDITIONS
...
%       disp(on_edge)
  ...
  if(~isempty(CCE))
    ...
    for( ii = 1:size(CCE,1) )
      %Calcula os coefs do plano que contém o circulo
      [aplane, bplane, cplane] = bm_planecoeffs(C(P(CCE(ii,1)),:),C(P(CCE(ii,2)),:));
      if abs(cplane) <= 1e-5
        cplane = 1e-4;
      end
      %calcula a distancia dos pontos em relacao ao plano
      [plane_distance] = bm_point2planedist(V,aplane, bplane, cplane, 0);
      %cria o terceiro ponto que forma o triangulo
      [xp, yp, zp] = bm_createtriangle(C(P(CCE(ii,1)),:),C(P(CCE(ii,2)),:),1);
      P_trig = [xp yp zp];
      %projeta os pontos no plano criado
      V_proj = bm_projpoint2plane(V,aplane,bplane,cplane,0);
      %calcula as coordenadas baricentricas dos pontos projetados
      Bar_coords = barycentric_coordinates(V_proj,ones(size(V_proj,1),1)*C(P(CCE(ii,1)),:),ones(size(V_proj,1),1)*C(P(CCE(ii,2)),:),ones(size(V_proj,1),1)*P_trig);
      Bar_1 = Bar_coords(:,1);
      Bar_2 = Bar_coords(:,2);
      Bar_3 = Bar_coords(:,3);
      
%       [t,sqr_d] = project_to_lines(V,C(P(CCE(ii,1)),:),C(P(CCE(ii,2)),:));
            t = bm_intersec_retas(V_proj,C(P(CCE(ii,1)),:),C(P(CCE(ii,2)),:));

      on_edge = ((t > -1e-6) & (t < 1+1e-6) & (abs(plane_distance) < h) & ...
    ((Bar_1 > -1e-5) & (Bar_2 > -1e-5) & (Bar_3 > -1e-5) )  & (Bar_1+Bar_2+Bar_3 <= 1+1e-5));
% 
%       [t,sqr_d] = project_to_lines(V,C(P(CCE(ii,1)),:),C(P(CCE(ii,2)),:));
%       on_edge = ((abs(sqr_d) < h*sqr_d_tol) & ((t > -1e-10) & (t < (1+1e-10))));
