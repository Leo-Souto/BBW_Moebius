...
handles.all_handles{end+1} = bm_handle(handles.num_of_handles,equi_coord,equi_coord,'Point',0);
...
handles.all_handles{handles.num_of_handles} = bm_handle(handles.num_of_handles,[equi_coord;...
        mid_point;equi_coord2],equi_coord_par,'Curved',0);
handles.all_handles{handles.num_of_handles}.proportion = 0.5;
...
%     e_coord = sphere2equi(handles.V(dsearchn(handles.V,equi2sphere(equi_coord)),:));
    ...
    if isequal(handles.button_value_handle,'closed_cage_handle')
        handles.all_handles{end+1} = bm_handle(handles.num_of_handles,equi_coord,equi_coord,'Closed_Cage',handles.plot_cage_tag_counter);
    end
    handles.all_handles{end}.cage_tag = handles.cage_tag_counter;
...
handles.all_cages = handles.new_cages;
handles.all_handles = handles.new_handle_order;
%% this creates a cage-edge list with the weights
handles.cage_edge_list = {};
handles.handle_to_edge_list = [];
if ~isempty(handles.new_cages)
    for i = 1:length(handles.new_cages)
        for j = 1:length(handles.new_cages{i})-1
            index1 = handles.new_cages{i}(j);
            index2 = handles.new_cages{i}(j+1);
            pos1 = handles.new_handle_order{index1}.position(1,:);
            pos2 = handles.new_handle_order{index2}.position(1,:);
            sphere_coord = equi2sphere(pos1);
            sphere_coord2 = equi2sphere(pos2);
            t = linspace(0,1,100);
                    parametric = [];
        ...
        parametric(:,3) = parametric(:,3)./norma;
        equi_coord_par = sphere2equi(parametric);
        handles.cage_edge_list{end+1} = bm_cage_edge([index1 index2],equi_coord_par);
        handles.handle_to_edge_list = [handles.handle_to_edge_list; index1 length(handles.cage_edge_list); index2 length(handles.cage_edge_list)];
        end
    end
end
for i = 1:handles.num_of_handles
   if isequal(handles.new_handle_order{i}.type,'Closed_Cage')
